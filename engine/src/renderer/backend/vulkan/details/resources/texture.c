...
void createVulkanTextures(VulkanContext* context, VkCommandPool cmdPool, DescriptorSetConfig* config, Texture** outTextures, u16* outCount){
    ...
    for(u8 i=0; i< config->bindingsCount; i++){
        VkDescriptorType bindingType = get_vulkan_descriptor_type(config->bindings[i].type);
        if(bindingType == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER){
            textureindx++;
        }
    }
    *outCount = textureindx;
    *outTextures = malloc(sizeof(Texture) * textureindx);
    ...
    for(u8 i=0; i< config->bindingsCount; i++){
        ...
        if(bindingType == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER){
            u16 mipLevels = 1;
            createTextureImage(context->gpu, context->device, cmdPool, context->queue.graphics, config->bindings[i].defaultTexturePath, &mipLevels, &(*outTextures)[textureindx].image, &(*outTextures)[textureindx].memory);
            createTextureImageView(context->device, (*outTextures)[textureindx].image, mipLevels, &(*outTextures)[textureindx].view);
            createTextureImageSampler(context->gpu, context->device, mipLevels, &(*outTextures)[textureindx].sampler);
            textureindx++;
        }
    }
}
...
void createTextureImage(VkPhysicalDevice gpu, VkDevice device, VkCommandPool cmdPool, VkQueue queue, String path, u16* outMipLevels, VkImage* outTextureImage, VkDeviceMemory* outTextureMemory){
    ...
    //TODO: move the texture path to config, (mabe move the whole image loading to the asset manager)
    //"./../resources/textures/viking_room.png"
    stbi_uc* pixels = stbi_load(path.val, &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
    ...
    *outMipLevels = (u16)FLOOR(log2(MAX(texWidth, texHeight))) + 1;
    ...
    createVulkanBuffer(gpu, device, imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &stagingBuffer, &stagingBufferMemory);

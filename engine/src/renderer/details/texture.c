...
/// @param pixels if 0, the texture will be created with a default color (BLACK)
/// @param out needs to have the width, height and mipLevels set before calling this function
/// @param out->image, out->memory, out->imageView, out->sampler will be set to the created texture
...
void createTextureImage(VkPhysicalDevice gpu, VkDevice device, VkCommandPool cmdPool, VkQueue queue, void *pixels, Texture *outTexture)
{
    ...
    if (pixels)
    {
        memcpy(data, pixels, (u64)imageSize);
    }
    else
    {
        memset(data, 0, (u64)imageSize);
    }

    vkUnmapMemory(device, stagingBufferMemory);

    outTexture->mipLevels = outTexture->mipLevels > 0 ? outTexture->mipLevels : 1;
    createImage(
        gpu, device, 
...
void fillTextureRegion(Texture* texture, void* pixels, u32 x, u32 y, u32 width, u32 height) {
    ...
    // Step 1: Create a staging buffer for the pixel data
    VkBuffer stagingBuffer;
    VkDeviceMemory stagingBufferMemory;

    // Calculate the size of the region to update
    VkDeviceSize regionSize = width * height * 4; // RGBA format (4 channels)

    createBuffer(rc->gpu, rc->device, regionSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                 &stagingBuffer, &stagingBufferMemory);

    // Copy the pixel data into the staging buffer
    void* data;
    vkMapMemory(rc->device, stagingBufferMemory, 0, regionSize, 0, &data);
    memcpy(data, pixels, (size_t)regionSize);
    vkUnmapMemory(rc->device, stagingBufferMemory);

    // Step 2: Begin a one-time command buffer for updating the texture
    VkCommandBuffer commandBuffer = beginSingleTimeCommands(rc->device, rc->commandPool);

    // Step 3: Transition the texture image layout to VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL
    ...
    // Step 4: Define the region to update in the texture
    ...
    // Step 5: Transition the texture image layout back to VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
    barrier.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
    barrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
    barrier.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
    barrier.newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    ...
    endSingleTimeCommands(rc->device, rc->commandPool, rc->queue.graphics, &commandBuffer);

    // Step 6: Clean up the staging buffer
    vkDestroyBuffer(rc->device, stagingBuffer, NULL);
    vkFreeMemory(rc->device, stagingBufferMemory, NULL);
}